...
STATUSES = ["neu", "Vorbereitung", "Antrag", "in Bearbeitung", "RiVo", "Termin", "Abgeschlossen", "nicht erreicht", "Abgelegt"]
...
def bulk_update_status(ids: Iterable[int], status: str) -> int:
    ids = list(ids)
    if not ids:
        return 0
    con = connect()
    q = f"UPDATE leads SET status = ?, updated_at = ? WHERE id IN ({','.join(['?']*len(ids))})"
    cur = con.execute(q, [status, _now(), *ids])
    con.commit()
    con.close()
    for lead_id in ids:
        save_version(int(lead_id))
    return cur.rowcount
